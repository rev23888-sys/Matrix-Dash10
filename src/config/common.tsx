...
export const config: AppConfig = {
  name: 'Matrix Bot',
  icon: MatrixIcon,
  inviteUrl: 'https://tinyurl.com/Invite-Matrix',
  guild: {
    // Show ALL servers where user has ADMINISTRATOR or MANAGE_GUILD permissions.
    // Owner flag (guild.owner) is checked separately in the UI.
    filter: (guild) => {
      const perms = BigInt(guild.permissions ?? '0');
      const hasAdmin       = (perms & BigInt(PermissionFlags.ADMINISTRATOR))  !== BigInt(0);
      const hasManageGuild = (perms & BigInt(PermissionFlags.MANAGE_GUILD))   !== BigInt(0);
